<template>
    <view class="center">
        <!-- 登录组件 -->
        <wx-login v-if="!hasLogin" @loginSuccess="handleLoginSuccess"></wx-login>
        
        <!-- 用户信息区域 -->
        <view v-else class="user-profile">
            <view class="user-header">
                <image class="avatar" v-if="userInfo.avatarUrl" :src="userInfo.avatarUrl" mode="aspectFill"></image>
            ...
            <view class="user-stats">
                <view class="stat-item" @click="navTo('/pages/ucenter/collection/collection')">
                    <text class="number">{{collectionCount}}</text>
                    <text class="label">收藏</text>
                </view>
                <view class="stat-item" @click="navTo('/pages/ucenter/history/history')">
                    <text class="number">{{historyCount}}</text>
                    <text class="label">浏览历史</text>
                </view>
                <view class="stat-item" @click="navTo('/pages/ucenter/preference/preference')">
                    <text class="number">{{preferenceCount}}</text>
            ...
            <view class="feature-list">
                <view class="feature-group">
                    <view class="feature-item" @click="navTo('/pages/ucenter/collection/collection')">
                        <uni-icons type="star-filled" size="24" color="#FFC107"></uni-icons>
                        <text class="feature-text">我的收藏</text>
                        <uni-icons type="right" size="16" color="#999"></uni-icons>
                    </view>
                    <view class="feature-item" @click="navTo('/pages/ucenter/history/history')">
                        <uni-icons type="clock-filled" size="24" color="#FF9800"></uni-icons>
                        <text class="feature-text">浏览历史</text>
                        <uni-icons type="right" size="16" color="#999"></uni-icons>
                    </view>
                    <view class="feature-item" @click="navTo('/pages/ucenter/preference/preference')">
                        <uni-icons type="heart-filled" size="24" color="#F44336"></uni-icons>
                        <text class="feature-text">口味偏好设置</text>
                        <uni-icons type="right" size="16" color="#999"></uni-icons>
                    </view>
                </view>

                <view class="feature-group">
                    <view class="feature-item" @click="navTo('/pages/ucenter/settings/settings')">
                        <uni-icons type="gear-filled" size="24" color="#2196F3"></uni-icons>
                        <text class="feature-text">系统设置</text>
                        <uni-icons type="right" size="16" color="#999"></uni-icons>
                    </view>
                    <view class="feature-item" @click="handleLogout">
                        <uni-icons type="poweroff" size="24" color="#757575"></uni-icons>
...
export default {
    ...
    methods: {
        ...
        // 加载用户数据
        async loadUserData() {
            // 这里可以加载用户的收藏数、浏览历史数和偏好设置数
            const db = uniCloud.database();
            
            try {
                // 获取收藏数量
                const collectionRes = await db.collection('user-collections')
                    .where(`user_id == $cloudEnv_uid`)
                    .count();
                this.collectionCount = collectionRes.total;

                // 获取浏览历史数量
                const historyRes = await db.collection('browse-history')
                    .where(`user_id == $cloudEnv_uid`)
                    .count();
                this.historyCount = historyRes.total;

                // 获取口味偏好数量
                const preferenceRes = await db.collection('taste-preferences')
                    .where(`user_id == $cloudEnv_uid`)
                    .count();
                this.preferenceCount = preferenceRes.total;
